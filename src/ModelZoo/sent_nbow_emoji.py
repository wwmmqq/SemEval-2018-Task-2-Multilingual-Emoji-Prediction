...
class Model(BaseModel):
    def __init__(self, cfg, we, emoji):
        ...
        self.emoji_w = tf.Variable(initial_value=emoji, trainable=False)
    ...
    def _build_model(self):
        ...
        with tf.name_scope("mlp"):
            mlp = tf_ops.dense_layer(
                avg_out, self.dim_word, 300, activation=tf.nn.relu, scope="mlp")

        with tf.name_scope('logits'):
            # self.logits = tf_ops.dense_layer(mlp, self.n_mlp, self.n_class, scope="logits")
            # self.logits = tf.matmul(mlp, self.emoji_w)
